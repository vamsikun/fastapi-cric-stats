...
def getSQLForBowlerBestAverage(season, limit=10, minBalls=60):
    selectStmt = select(
        func.row_number()
        .over(
            order_by=nulls_last(
        ...
        cast(
            cast(
                # have to conver this to integer and then to string to remove double dots issue
                cast(func.sum(bowlerStatsTable.c.legal_deliveries) / 6, Integer),
                String,
            )
            + "."
    ...
    selectStmt = selectStmt.order_by(nulls_last(asc("avg")))
