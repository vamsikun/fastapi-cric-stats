...
def getSQLForBatterMostSixes(season, limit=10):
    selectStmt = select(
        func.row_number()
        .over(order_by=nulls_last(desc(func.sum(batStats.c.sixes))))
        .label("pos"),
        batStats.c.player,
    ...
    selectStmt = selectStmt.order_by(nulls_last(desc("sixes")))
